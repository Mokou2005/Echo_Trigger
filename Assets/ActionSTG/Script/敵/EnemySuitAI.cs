...
namespace StateMachineAI
{
    ...
    public class EnemySuitAI : StatefulObjectBase<EnemySuitAI, AIState>
    {
        ...
        */
        ...
        public bool AddStateByName(string className)
        {
            try
            {
                ...
                // 型チェック
                if (!typeof(State<EnemySuitAI>).IsAssignableFrom(stateType))
                {
                ...
                // コンストラクタ取得
                ConstructorInfo constructor = stateType.GetConstructor(new[] { typeof(EnemySuitAI) });
                if (constructor == null)
                {
                ...
                // インスタンス生成
                State<EnemySuitAI> stateInstance =
                    constructor.Invoke(new object[] { this }) as State<EnemySuitAI>;
